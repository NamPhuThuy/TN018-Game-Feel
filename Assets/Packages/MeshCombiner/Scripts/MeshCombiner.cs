...
public class MeshCombiner : MonoBehaviour
{
	...
	[SerializeField]
	private string folderPath = "_Game/_Scenes/Game/CombinedMeshes";
	[SerializeField]
	...
	[SerializeField]
    private List<MeshFilter> filteredMeshFilters;
	...
	public void DisableMeshFilter(bool tf)
	{
        foreach (var meshFilter in filteredMeshFilters)
        {
            meshFilter.GetComponent<MeshRenderer>().enabled = tf;
        }
		bool tf2 = !tf;
		filteredMeshFilters[0].GetComponent<MeshRenderer>().enabled = tf2;
    }
    ...
    private MeshFilter[] GetMeshFiltersToCombine()
    {
        ...
        if (meshFiltersToSkip == null)
            return meshFilters;

        // Use HashSet for efficient lookups
        HashSet<MeshFilter> meshFiltersToSkipSet = new HashSet<MeshFilter>(meshFiltersToSkip.Where(mf => mf != null));

		meshFilters[0].GetComponent<MeshRenderer>().enabled = true;
		meshFilters[0].gameObject.isStatic = true;


        // Iterate through meshFilters and filter out unwanted ones
        filteredMeshFilters = new List<MeshFilter>();
        for (int i = 0; i < meshFilters.Length; i++)
        {
            MeshFilter meshFilter = meshFilters[i];
            if (meshFilter != null && !meshFiltersToSkipSet.Contains(meshFilter) && meshFilter.gameObject.isStatic && meshFilter.GetComponent<MeshRenderer>() != null && meshFilter.GetComponent<MeshRenderer>().enabled == true)
            {
